...
```{r echo=FALSE, Univariate_Plots}
table(PC$election_tp)

ggplot(aes(x = election_tp), data = PC) +
  geom_bar()
```
#### 44923 contributions were made to primary elections and 20545 contributions were made to general elections.
...
#### Seperating the bar plots of total contributions received by candidates into different political party definitely showed us a clear trend that Democratic candidates take smaller donations in a larger quantities, whereas Republican candidates takes bigger donations but in a smaller quantities.
...
### One major finding is that democratic candidates receive smaller donations but in a larger quantities, whereas the opposite is true for Republican candidates. 

### However, one limitations of this analysis is that since most of the variables are categorical, finding correlations is difficult.