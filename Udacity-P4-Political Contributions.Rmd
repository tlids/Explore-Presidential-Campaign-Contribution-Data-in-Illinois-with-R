...
```{r}
# Create a new variable for candidate's associated political party by creating a new function rnParty and applying that function
...
if(is.na(x)){
  NA
}else if(x == 'Carson, Benjamin S.'){
  'R'
}else if(x == 'Bush, Jeb'){
  'R'
}else if(x == 'Christie, Christopher J.'){
  'R'
}else if(x == 'Clinton, Hillary Rodham'){
  'D'
}else if(x == "Cruz, Rafael Edward 'Ted'"){
  'R'
}else if(x == 'Fiorina, Carly'){
  'R'
}else if(x == 'Huckabee, Mike'){
  'R'
}else if(x == 'Johnson, Gary'){
  'L'
}else if(x == 'Kasich, John R.'){
  'R'
}else if(x == 'Lessig, Lawrence'){
  'D'
}else if(x == 'Paul, Rand'){
  'R'
}else if(x == 'Rubio, Marco'){
  'R'
}else if(x == 'Sanders, Bernard'){
  'D'
}else if(x == 'Stein, Jill'){
  'G'
}else if(x == 'Trump, Donald J.'){
  'R'
}else if(x == 'Walker, Scott'){
  'R'
}else{
  NA
}
...
#### Features like contributor name, contributor city, contributor zipcode, contributor employer, contributor occupation, contribution, election type, candidate's political party can provide further insights of the dataset. We can find relationship between these features and the features I mentioned in the last question. For example, we can try to determine if occupation has an effect on contribution amount.
...
#### I noticed that the contributions come from many cities but many of them concentrate on one city - chicago. Similar situation happens to occupation.
...
ggplot(aes(x = cand_nm, y = contb_receipt_amt), data = subset(PC, !is.na(PC$contb_receipt_amt))) +
  stat_summary_bin(aes(y = contb_receipt_amt), fun.y = 'sum', geom = 'bar', data = subset(PC, !is.na(PC$contb_receipt_amt))) +
  coord_flip()
```
...
#### One obvious pattern is that contribution amounts vary significantly across different candidate in terms of total, median, etc. Another pattern is that in general, more contributions go toward primary election rather than general election. More money is dedicated to make the candiates the party nominee perhaps since only one candidate from each party actually compete in the general election. 
...
#### Since we only have one numeric variable - contribution amount, we cannot find the correlation.
...
ggplot(aes(x = cand_nm, y = contb_receipt_amt), data = PC) +
  stat_summary_bin(aes(y = contb_receipt_amt, fill = election_tp), fun.y = 'sum', geom = 'bar') +
  coord_flip()
```
...
#### Seprating the bar plot of total contributions received by candidates into different political party definitely showed us a clear trend that Democratic candidates take smaller donations in a larger quantities, whereas Republican candidates takes bigger donations but in a smaller quantities.

### Were there any interesting or surprising interactions between features?
#### I thought that the interaction between total contribution amount and election type is quite interesting. It confirms our intuition that only the major candidates who become the party nominees would receive donations for general election.
...
scale_y_log10()+
  ...
  ylab('Contribution Amount Received')
```
...
stat_summary_bin(aes(y = contb_receipt_amt, fill = election_tp), fun.y = 'sum', geom = 'bar') +
coord_flip() +
  ggtitle('Bar Plots of Total Contribution Amount Received for Each Candidate, with Election Type Information') +
  ylab('Total Contribution Amount Received') +
  xlab('Candidate Name') +
...
ggplot(aes(x = cand_nm, y = contb_receipt_amt), data = PC) +
  ...
  facet_wrap(~cand_nm_party) +
  coord_flip() +
  ggtitle('Bar Plots of Total Contribution Amount Received for Each Candidate, Faceted by the Political Parties of the Candidates') +
  xlab('Candidate Name') +
  ylab('Total Contribution Amount')
```

### Description Three
#### This plot demonstrates the total amount of contributions received for each candidate, faceted by political parties of the candidates.
------

# Reflection
### Through doing univariate, bivariate and multivariate analysis, we gained a lot of insights about the political contributions made to 2016 presidential candidates in Illinois, including which candidate received the most contributions and total amount of contributions, summary statistics of contributions for each candidate, differences in contributions across election types and political parties. We learnt that democratic candidates receive smaller donations but in a larger quantities, whereas the opposite is true for Republican candidates. However, one limitations of this analysis is that since most of the variables are categorical, finding correlations is difficult.